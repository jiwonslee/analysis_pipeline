...
n_range = "1-" + str(n-1)
n_range_parts = [int(x) for x in n_range.split('-')]
start=n_range_parts[0]
end= start if len(n_range_parts) ==1 else n_range_parts[1]
n_range=range(start,end+1)
n_range = [str(x) for x in n_range]

for seg in n_range:
    cmd = ' '.join(['bsub -q short Rscript', rscript, configfile, '--segment ' + seg])
    os.system(cmd)
